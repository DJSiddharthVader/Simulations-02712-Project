...
"""
Script for cleaning the HMP2 data for easier parsing and
...
def filter_abundances(thresh):
    # read data into pandas
    df = pd.read_csv('./Data/taxonomic_profiles.tsv', sep='\t')
    print(df.shape)
    otus_non_zero = (df != 0).apply(sum, axis=1)
    df = df[otus_non_zero / df.shape[1] > thresh]
    df.to_csv('./Data/filtered_taxonomic_profiles.tsv', sep='\t', index=False)
    print(df.shape)
